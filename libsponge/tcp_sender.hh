...
class TCPSender{
    class Timer{
        ...
        public:
            Timer(uint32_t initial_time, bool is_expired, bool is_stopped)
            : _remaining_time(initial_time)
            , _is_expired(is_expired)
            , _is_stopped(is_stopped){}

            void time_expired(uint32_t time_has_expired){
                if(_is_stopped) return;
                if(time_has_expired >= _remaining_time) {
                    _is_expired = true;
                    _remaining_time = 0;
                }
                else _remaining_time -= time_has_expired;
            }

            bool is_expired() {return _is_expired && !_is_stopped;}
            bool is_stop() {return _is_stopped;}
    ...
    bool FIN_SENT() const {
        return stream_in().eof() && next_seqno_absolute() == stream_in().bytes_written() + 2
                && bytes_in_flight() >0 ;
    }
